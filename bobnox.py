...
FONT_FILES = {
    ...
    "Geist Light": os.path.join(GEIST_STATIC, "Geist-Light.ttf"),
    "Geist Mono": os.path.join(GEIST_STATIC, "Geist-Regular.ttf"),
}

def register_geist_fonts():
    """Check if Geist fonts are available. Returns True if installed system-wide."""
    try:
        test = tkfont.Font(family="Geist", size=12)
        family = test.actual("family")
        return "geist" in family.lower()
    except Exception:
        return False


def install_geist_fonts():
    """Attempt to install Geist fonts for the current user."""
    installed = 0
        ...
        if os.path.exists(path):
            ...
            if not dest.exists():
                try:
                    ...
                    logger.warning(f"Failed to copy font {name}: {e}")

    if installed > 0:
        try:
            subprocess.run(["fc-cache", "-f", str(local_dir)], capture_output=True, timeout=10)
            logger.info(f"Installed {installed} Geist fonts to {local_dir}")
            # Wait for font cache to be recognized
            import time
            time.sleep(0.5)
        except Exception:
            logger.warning("fc-cache failed, fonts may not be available until reboot")
        return True
    return False
...
class SettingsDialog(ctk.CTkToplevel):
    ...
    def __init__(self, parent, config: dict, on_save_callback, fonts_registered: bool = False):
        ...
        self.FONT_LABEL = ("Geist", 13) if fonts_registered else ("Helvetica", 13)
        self.FONT_TITLE = ("Geist", 18, "bold") if fonts_registered else ("Helvetica", 18, "bold")
        self.BG_CARD = "#1A1A1A"
    ...
    def _create_widgets(self):
        ...
        ctk.CTkButton(btn_frame, text="Save", font=self.FONT_LABEL, fg_color=self.ACCENT, hover_color="#004BB3", height=36, corner_radius=8, command=self._save).pack(side="left", padx=(0, 8))
        ctk.CTkButton(btn_frame, text="Cancel", font=self.FONT_LABEL, fg_color="#2C2C2E", hover_color="#3A3A3C", text_color=self.TEXT_MAIN, height=36, corner_radius=8, command=self.destroy).pack(side="left")
...
class BoBnoxApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        # Register Geist fonts - install if not available
        self.fonts_registered = register_geist_fonts()
        if not self.fonts_registered:
            logger.info("Geist fonts not found, attempting installation...")
            install_geist_fonts()
            # Re-check after installation
            self.fonts_registered = register_geist_fonts()
        if not self.fonts_registered:
            logger.warning("Geist fonts unavailable, using system fallback")
        ...
        # Fonts - Geist with fallback
        if self.fonts_registered:
            self.FONT_TITLE = ("Geist", 26, "bold")
            self.FONT_SUBTITLE = ("Geist", 12)
            self.FONT_LABEL = ("Geist", 13)
            self.FONT_BUTTON = ("Geist", 13, "bold")
            self.FONT_CONSOLE = ("Geist", 12)
        else:
            self.FONT_TITLE = ("Helvetica", 26, "bold")
            self.FONT_SUBTITLE = ("Helvetica", 12)
            self.FONT_LABEL = ("Helvetica", 13)
            self.FONT_BUTTON = ("Helvetica", 13, "bold")
            self.FONT_CONSOLE = ("Courier", 12)
    ...
    def _create_widgets(self):
        ...
        title_label = ctk.CTkLabel(brand_card, text="boBnox", text_color=self.TEXT_MAIN, font=self.FONT_TITLE)
        title_label.pack(anchor="w", padx=24, pady=(20, 4))

        subtitle_label = ctk.CTkLabel(brand_card, text="Organize your files into categorized folders", text_color=self.TEXT_MUTED, font=self.FONT_LABEL)
        subtitle_label.pack(anchor="w", padx=24, pady=(0, 20))

        # --- CARD 2: Options (Top Right) ---
        ...
        options_title = ctk.CTkLabel(config_card, text="Options", text_color=self.TEXT_MUTED, font=self.FONT_SUBTITLE)
        options_title.pack(anchor="w", padx=24, pady=(16, 8))

        self.dry_run_check = ctk.CTkCheckBox(
            config_card, text="Dry Run (Preview only)", variable=self.dry_run_var,
            font=self.FONT_LABEL, text_color=self.TEXT_MAIN,
            hover_color=self.ACCENT_BLUE, fg_color=self.ACCENT_BLUE,
            checkbox_width=18, checkbox_height=18
        )
        self.dry_run_check.pack(anchor="w", padx=24, pady=6)

        self.recursive_check = ctk.CTkCheckBox(
            config_card, text="Include Subdirectories", variable=self.recursive_var,
            font=self.FONT_LABEL, text_color=self.TEXT_MAIN,
            hover_color=self.ACCENT_BLUE, fg_color=self.ACCENT_BLUE,
        ...
        # --- CARD 3: Target Directory (Middle Row) ---
        ...
        path_label = ctk.CTkLabel(path_card, text="Target Directory", text_color=self.TEXT_MUTED, font=self.FONT_SUBTITLE)
        path_label.pack(anchor="w", padx=24, pady=(14, 6))

        path_entry_frame = ctk.CTkFrame(path_card, fg_color="transparent")
        path_entry_frame.pack(fill="x", padx=20, pady=(0, 18))

        self.path_entry = ctk.CTkEntry(
            path_entry_frame, placeholder_text="Select a directory path...",
            fg_color="#0D0D0D", border_color="#2C2C2E", text_color=self.TEXT_MAIN,
            font=self.FONT_CONSOLE, height=38, corner_radius=8, textvariable=self.path_var
        )
        self.path_entry.pack(side="left", fill="x", expand=True, padx=(0, 12))

        # Browse button with icon
        browse_kwargs = dict(
            text="Browse", font=self.FONT_BUTTON,
            fg_color=self.ACCENT_BLUE, hover_color="#004BB3",
        ...
        self.browse_btn = ctk.CTkButton(path_entry_frame, **browse_kwargs)
        self.browse_btn.pack(side="left")

        # --- CARD 4: Action Controls (Grid Layout) ---
        ...
        actions_card.grid_columnconfigure((0, 1, 2, 3), weight=1, uniform="buttons")
        actions_card.grid_columnconfigure(4, weight=1, uniform="buttons")
        ...
        organize_kwargs = dict(
            text="Organize", font=self.FONT_BUTTON,
            fg_color=self.ACCENT_GREEN, hover_color="#1B9E46", text_color="#0D0D0D",
            height=36, corner_radius=8, command=self._start_organizing
        )
        if self.organize_img:
            organize_kwargs["image"] = self.organize_img
            organize_kwargs["compound"] = "left"
        self.organize_btn = ctk.CTkButton(actions_card, **organize_kwargs)
        self.organize_btn.grid(row=0, column=0, padx=(16, 6), pady=14, sticky="ew")

        self.undo_btn = ctk.CTkButton(
            actions_card, text="Undo", font=self.FONT_BUTTON,
            fg_color="#2C2C2E", hover_color="#3A3A3C", text_color=self.TEXT_MAIN,
            height=36, corner_radius=8, command=self._undo_action, state="disabled"
        )
        self.undo_btn.grid(row=0, column=1, padx=6, pady=14, sticky="ew")

        self.settings_btn = ctk.CTkButton(
            actions_card, text="Settings", font=self.FONT_BUTTON,
            fg_color="#2C2C2E", hover_color="#3A3A3C", text_color=self.TEXT_MAIN,
            height=36, corner_radius=8, command=self._open_settings
        )
        self.settings_btn.grid(row=0, column=2, padx=6, pady=14, sticky="ew")

        self.open_folder_btn = ctk.CTkButton(
            actions_card, text="Open Folder", font=self.FONT_BUTTON,
            fg_color="#2C2C2E", hover_color="#3A3A3C", text_color=self.TEXT_MAIN,
            height=36, corner_radius=8, command=self._open_folder
        )
        self.open_folder_btn.grid(row=0, column=3, padx=6, pady=14, sticky="ew")

        self.clear_btn = ctk.CTkButton(
            actions_card, text="Clear Console", font=self.FONT_BUTTON,
            fg_color="transparent", hover_color="#2C2C2E", text_color=self.ACCENT_CORAL,
            border_color=self.ACCENT_CORAL, border_width=1, height=36, corner_radius=8,
        ...
        self.progress_bar = ctk.CTkProgressBar(
            terminal_card, height=4, fg_color="#2C2C2E", progress_color=self.ACCENT_BLUE
        )
        self.progress_bar.pack(fill="x", padx=24, pady=4)
        self.progress_bar.set(0.0)

        self.console = ctk.CTkTextbox(
            terminal_card, fg_color="#0D0D0D", text_color="#A9A9B2",
            font=self.FONT_CONSOLE, corner_radius=8, border_color="#2C2C2E", border_width=1
        )
    ...
    def _open_settings(self):
        SettingsDialog(self, self.config, self._on_settings_save, self.fonts_registered)
