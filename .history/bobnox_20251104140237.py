...
# removed drag-and-drop dependency to keep UI simple
import threading
import io
import base64
# drag & drop removed; no need for shlex/urllib parsing
...
class FileOrganizerApp(tk.Tk):
    ...
    def __init__(self):
        ...
        # keep the window compact but usable
        self.geometry("520x360")
        self.configure(bg="#1E1E1E")
        self.path_var = tk.StringVar()
        self.status_var = tk.StringVar(value="Ready. Select a folder or browse to begin.")
    ...
    def create_widgets(self):
        ...
        # Title / instruction area (simplified, no drag & drop)
        ttk.Label(main_frame, text="boBnox — Select a folder to organize", style="Title.TLabel").grid(row=1, column=0, pady=(0, 8))

        # Separator or alternative instruction
        ttk.Label(main_frame, text="— OR —", foreground="#555555").grid(row=2, column=0, pady=2)
        
        # 3. Path Entry and Browse Button
        path_frame = ttk.Frame(main_frame)
        path_frame.grid(row=3, column=0, sticky="ew", pady=(6, 14))
        ...
        def make_icon_button(image=None):
            ...
            btn.grid(row=4, column=0, pady=(6, 12))
            return btn
        ...
        self.progress_bar.grid(row=5, column=0, sticky="ew", pady=(6, 6))
        ...
        # Save log button and status row
        controls_frame = ttk.Frame(main_frame)
        controls_frame.grid(row=7, column=0, sticky="ew", pady=(6, 0))
        controls_frame.grid_columnconfigure(0, weight=1)

        self.status_label = ttk.Label(controls_frame, textvariable=self.status_var, style="Status.TLabel")
        self.status_label.grid(row=0, column=0, sticky="w")

        save_btn = ttk.Button(controls_frame, text="Save Log", command=lambda: self.save_log_file(self.path_var.get() or None))
