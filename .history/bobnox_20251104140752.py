...
class FileOrganizerApp(tk.Tk):
    ...
    # Keep window compact by default
    ...
    def create_widgets(self):
        ...
        # Instruction label
        ttk.Label(main_frame, text="Select a folder or use Browse", style="Title.TLabel").grid(row=1, column=0, pady=(0, 8))
    ...
    browse_button = ttk.Button(path_frame, text="Browse...", command=self.select_directory)
        ...
        def make_icon_button(image=None):
            ...
            btn.grid(row=4, column=0, pady=(10, 8))
            return btn
    ...
    # Save Log button next to status
    status_frame = ttk.Frame(main_frame)
    status_frame.grid(row=7, column=0, sticky="ew", pady=(6,0))
    status_frame.grid_columnconfigure(0, weight=1)
    self.status_label = ttk.Label(status_frame, textvariable=self.status_var, style="Status.TLabel")
    self.status_label.grid(row=0, column=0, sticky="w")
    save_btn = ttk.Button(status_frame, text="Save Log", command=self._save_log_to_file)
    save_btn.grid(row=0, column=1, sticky="e")


    # --- UI EVENT HANDLERS ---

    # Drag-and-drop removed: keep UI simple and use Browse button
