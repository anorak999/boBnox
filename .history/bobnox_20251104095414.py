...
class FileOrganizerApp(TkinterDnD.Tk):
    ...
    def setup_styles(self):
        ...
        style.configure("TLabel", background=BG_DARK, foreground=FG_LIGHT, font=("Inter", 12))
        style.configure("Title.TLabel", font=("Inter", 24, "bold"), foreground=ACCENT_COLOR)
        style.configure("Status.TLabel", background=BG_DARK, foreground="#999999", font=("Inter", 10, "italic"))
        
        # Entry/Input
        style.configure("TEntry", fieldbackground=BG_MID, foreground=FG_LIGHT, borderwidth=0, relief="flat", padding=8)
    ...
    def create_widgets(self):
        ...
        # 4. Organize Button - prefer SVG icon if available, otherwise fall back to text
        assets_dir = os.path.join(os.path.dirname(__file__), 'assets')
        svg_name = 'Sort--Streamline-Solar.svg'
        svg_path = os.path.join(assets_dir, svg_name)

        def make_text_button():
            btn = ttk.Button(main_frame, text="🚀 START ORGANIZATION", command=self.start_organizing_thread)
            btn.grid(row=4, column=0, pady=(10, 20), sticky="ew", ipady=12)
            return btn
        ...
        if HAS_SVG_SUPPORT and os.path.exists(svg_path):
            try:
                # render SVG to PNG bytes and load into PhotoImage
                ...
                # Create a button with the image only. Keep it stretchable like before.
                self.organize_button = ttk.Button(main_frame, image=self.organize_img, command=self.start_organizing_thread)
                self.organize_button.grid(row=4, column=0, pady=(10, 20), sticky="ew", ipady=12)
            except Exception:
                # If anything goes wrong, fall back to text button
                self.organize_button = make_text_button()
        else:
            self.organize_button = make_text_button()
